...
public class LoginServlet extends HttpServlet implements IConstants {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (StringUtils.isStringEmpty(email) || StringUtils.isStringEmpty(password)) {
            RequestDispatcher rd = request.getRequestDispatcher(IConstants.LOGIN_PAGE);
            ...
            if (user == null) {
                ...
                if (user.getAuth().equals(IConstants.USER_TYPE_ADMIN)) {
                    RequestDispatcher rd = request.getRequestDispatcher(IConstants.ADMIN_HOME_PAGE_SERVLET);
                    rd.forward(request, response);
                } else if (user.getAuth().equals(IConstants.USER_TYPE_GENERAL_USER)) {
                    RequestDispatcher rd = request.getRequestDispatcher(IConstants.SERVE_FAN_HOME_PAGE);
                    rd.forward(request, response);
                } else {
                    RequestDispatcher rd = request.getRequestDispatcher(IConstants.LOGIN_PAGE);
                    rd.forward(request, response);
